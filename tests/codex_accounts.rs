...
for line in sys.stdin:
    ...
    if method == 'initialize': result = {}
        ...
        if os.path.exists(root + '/fail-fetch') or os.path.exists(home + '/fail-fetch'): sys.exit(1)
...
fn invoke_for_pane(root: &Path, args: &[&str], pane: &str) {
    ...
    if args[0] == "startup" {
        // Upstream startup restores one detached watcher. Let its idle pass
        // finish before inspecting logs or removing the owned fixture.
        let cache = CacheStore::new(root.join("cache"));
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(5);
        loop {
            let inventory_calls = fs::read_to_string(root.join("herdr.log"))
                .unwrap_or_default()
                .lines()
                .filter(|line| *line == "[\"agent\", \"list\"]")
                .count();
            if inventory_calls >= 2 && cache.try_lock_named("turn.lock").unwrap().is_some() {
                break;
            }
            assert!(
                std::time::Instant::now() < deadline,
                "startup watcher did not settle"
            );
...
fn followup_generation_changes_cannot_merge_old_windows_or_last_good_quota() {
    ...
    assert!(
        serde_json::from_slice::<Value>(&fs::read(&a_snapshot).unwrap()).unwrap()
...
#[test]
fn scoped_cache_replaces_omitted_windows_and_retains_failed_fetches_for_only_that_account() {
    let dir = tempdir().unwrap();
    let root = dir.path();
    setup(root);
    refresh(root);
    for marker in ["omit-week", "fail-fetch"] {
        ...
        refresh(root);
        let reports = writes(root);
        for (pane, quota) in [("w1:p1", "5h 88%"), ("w1:p2", "5h 27%")] {
            let report = reports
                .iter()
                .find(|args| args[2] == pane)
                .unwrap()
            ...
            assert!(
                !report.contains("7d 88%") && !report.contains("7d 27%"),
                "{report}"
            );
        }
    }
...
#[test]
fn failed_account_attempt_cannot_contaminate_or_debounce_another_home() {
    let dir = tempdir().unwrap();
    let root = dir.path();
    setup(root);
    fs::write(root.join("a/fail-fetch"), "").unwrap();
    refresh(root);
    let reports = writes(root);
    for pane in ["w1:p1", "w1:p3"] {
        assert!(reports
            .iter()
            .find(|report| report[2] == pane)
            .unwrap()
            .join(" ")
            .contains("N/A"));
    }
    ...
    refresh(root);
    assert_eq!(fs::read_to_string(root.join("codex.log")).unwrap(), calls);
    // B's rotation can refresh immediately while A's failure stays debounced.
    fs::write(
        root.join("b/auth.json"),
        json!({"tokens":{"account_id":"account-b"},"synthetic_rotation":1}).to_string(),
    )
    .unwrap();
    refresh(root);
    let calls = fs::read_to_string(root.join("codex.log")).unwrap();
    assert_eq!(calls.lines().filter(|id| *id == "account-a").count(), 1);
    assert_eq!(calls.lines().filter(|id| *id == "account-b").count(), 2);
}

#[test]
fn active_watch_batches_only_working_homes_and_their_siblings() {
    use std::process::Stdio;
    use std::time::{Duration, Instant};
    // One active home must not fetch B; two active homes still fetch A once
    // for its two panes. Stop the real watcher through its local stop marker.
    for both in [false, true] {
        ...
        if both {
            inventory["agents"][1]["agent_status"] = "working".into();
        }
        fs::write(root.join("inventory.json"), inventory.to_string()).unwrap();
        let mut child = command(root)
            .args(["watch", "--provider", "codex", "--interval-seconds", "30"])
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
            .unwrap();
        let deadline = Instant::now() + Duration::from_secs(8);
        let expected = if both { 3 } else { 2 };
        let mut observed = false;
        while Instant::now() < deadline {
            if root.join("herdr.log").exists() && writes(root).len() == expected {
                observed = true;
                break;
            }
        ...
        assert!(observed, "watch did not publish its selected accounts");
        assert!(
            status.is_some_and(|status| status.success()),
            "watch did not honor its stop marker"
        );
        let collected = fs::read_to_string(root.join("codex.log")).unwrap();
        assert_eq!(collected.lines().filter(|id| *id == "account-a").count(), 1);
        assert_eq!(
            collected.lines().filter(|id| *id == "account-b").count(),
            usize::from(both)
        );
        let reports = writes(root);
        assert_eq!(reports.len(), expected);
        for (pane, percent) in [("w1:p1", "88%"), ("w1:p3", "88%"), ("w1:p2", "27%")] {
            if pane == "w1:p2" && !both {
                continue;
            }
            assert!(reports
        ...
        assert_eq!(
            calls
                .iter()
                .filter(|args| *args == &["agent", "list"])
                .count(),
            1
        );
        assert!(!calls
            .iter()
            .any(|args| args.starts_with(&["pane".into(), "read".into()])));
    }
}
