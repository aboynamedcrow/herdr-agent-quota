...
For isolated suite execution, remove inherited `BASH_ENV` and `ENV` from the
test command's environment. Non-interactive shell startup hooks may reorder
`PATH` ahead of a fixture's fake executables. The upgrade fixture explicitly
removes both before launching its installer; never run the real installer as
a substitute for a failing simulated-upgrade test.
...
same-credential last-good retention, and authoritative replacement of omitted API windows (no backfill).
...
Exercise the real watcher against synthetic inventory with one and then two
working homes. Require one inventory call and one collection per active home,
publication only to matching siblings, and prompt exit through the local stop
marker. Upstream's separate watcher tests cover settling and connection handoff.
